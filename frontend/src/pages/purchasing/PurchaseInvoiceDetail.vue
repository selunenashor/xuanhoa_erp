  ...
  <div class="space-y-6">
    ...
    <template v-else-if="invoice">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-center gap-4">
          ...
          <div>
            ...
            <p class="text-gray-600">Hóa đơn mua hàng từ {{ invoice.supplier_name }}</p>
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="bg-white rounded-lg shadow p-4">
          <h3 class="text-sm font-medium text-gray-500 mb-2">Nhà cung cấp</h3>
          <p class="text-lg font-semibold text-gray-900">{{ invoice.supplier_name }}</p>
          <p class="text-sm text-gray-500">{{ invoice.supplier }}</p>
        </div>
      ...
      <div class="bg-white rounded-lg shadow p-4">
        <div class="flex flex-wrap items-center gap-3">
          ...
          <!-- Payment Button - Chỉ hiển thị khi đã duyệt VÀ còn nợ VÀ chưa có phiếu nhập kho -->
          <button
          ...
          <!-- Create Stock Entry Button - Chỉ hiển thị khi đã thanh toán đủ VÀ chưa có phiếu nhập kho -->
          <button
            v-if="invoice.docstatus === 1 && invoice.outstanding_amount <= 0 && !hasStockEntry && !invoice.update_stock"
            @click="createStockEntry"
            :disabled="actionLoading"
            class="inline-flex items-center gap-2 px-4 py-2 bg-success text-white rounded-lg hover:bg-green-600 disabled:opacity-50 transition-colors"
          >
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <!-- Completed Badge - Hiển thị khi đã thanh toán đủ VÀ đã có phiếu nhập kho -->
          <div
            v-if="isCompleted"
          ...
          <!-- Stock Entry Link - Hiển thị khi đã có phiếu nhập kho -->
          <router-link
            v-if="hasStockEntry"
          ...
          <!-- Cancel Button - Chỉ hiển thị khi draft HOẶC (đã duyệt + chưa thanh toán + chưa nhập kho) -->
          <button
            v-if="canCancel"
      ...
      <div v-if="invoice.stock_entries?.length > 0" class="bg-white rounded-lg shadow">
        <div class="px-6 py-4 border-b border-gray-200">
          <h2 class="text-lg font-semibold text-gray-900">Phiếu nhập kho liên quan</h2>
        </div>
    ...
    <div v-if="showPaymentModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div class="bg-white rounded-xl shadow-xl max-w-lg w-full p-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-4">Thanh toán hóa đơn</h3>
        
        <div class="space-y-4">
          <!-- Outstanding Amount Info -->
          <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <div class="flex justify-between items-center">
              <span class="text-blue-700">Số tiền còn nợ:</span>
              <span class="text-lg font-bold text-blue-700">{{ formatCurrency(invoice?.outstanding_amount) }}</span>
            </div>
          </div>
          ...
          <div>
            ...
            <input
              ...
              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary"
              placeholder="Số phiếu chi, số chứng từ ngân hàng..."
            />
          </div>
...
// Open payment modal with default values
const openPaymentModal = () => {
  paymentForm.amount = invoice.value?.outstanding_amount || 0
  paymentForm.mode = 'Cash'
  paymentForm.referenceNo = ''
  paymentForm.referenceDate = new Date().toISOString().split('T')[0]
  paymentError.value = ''
...
// Hóa đơn hoàn tất khi: đã duyệt + đã thanh toán đủ + đã có phiếu nhập kho
...
// Có thể hủy khi: draft HOẶC (đã duyệt + chưa thanh toán gì + chưa có phiếu nhập kho)
const canCancel = computed(() => {
  if (!invoice.value) return false
  if (invoice.value.docstatus === 2) return false // Đã hủy rồi
  if (invoice.value.docstatus === 0) return true // Draft có thể xóa
  // Đã duyệt: chỉ hủy được nếu chưa thanh toán gì và chưa nhập kho
...
const loadInvoice = async () => {
  ...
  try {
    const result = await purchaseInvoiceAPI.getDetail(route.params.name)
...
const submitInvoice = async () => {
  ...
  try {
    const result = await purchaseInvoiceAPI.submit(invoice.value.name)
...
const createStockEntry = async () => {
  ...
  try {
    const result = await purchaseInvoiceAPI.createStockEntry(invoice.value.name)
    
    if (result.success) {
      showToast('success', 'Đã tạo phiếu nhập kho thành công. Hóa đơn hoàn tất!')
    ...
    showToast('error', 'Có lỗi xảy ra khi tạo phiếu nhập kho')
  } finally {
    actionLoading.value = false
  }
}
...
const cancelInvoice = async () => {
  ...
  try {
    const result = await purchaseInvoiceAPI.cancel(invoice.value.name)
...
const processPayment = async () => {
  ...
  if (!paymentForm.amount || paymentForm.amount <= 0) {
    paymentError.value = 'Vui lòng nhập số tiền thanh toán'
    return
  }
  
  if (paymentForm.amount > invoice.value.outstanding_amount) {
    paymentError.value = 'Số tiền thanh toán không được lớn hơn số tiền còn nợ'
    return
  }
  ...
  try {
    const result = await purchaseInvoiceAPI.createPayment(
      invoice.value.name,
      paymentForm.amount,
      paymentForm.mode,
      paymentForm.referenceNo || null,
      paymentForm.mode !== 'Cash' ? paymentForm.referenceDate : null
    )
    ...
    if (result.success) {
      ...
      paymentError.value = result.message || 'Có lỗi xảy ra khi thanh toán'
    }
  } catch (e) {
    console.error('Error processing payment:', e)
    paymentError.value = 'Có lỗi xảy ra khi thanh toán. Vui lòng thử lại.'
  } finally {
    actionLoading.value = false
  }
}
