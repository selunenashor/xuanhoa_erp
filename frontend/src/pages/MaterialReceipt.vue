  ...
  <div class="min-h-screen bg-gray-100">
    ...
    <header class="bg-xh-red text-white shadow-lg">
      <div class="container mx-auto px-4 py-4 flex items-center">
        ...
        <h1 class="text-xl font-bold">Nhập kho</h1>
      </div>
    </header>
    ...
    <main class="container mx-auto px-4 py-6">
      <div class="bg-white rounded-lg shadow p-6">
        <form @submit.prevent="handleSubmit" class="space-y-6">
          ...
          <!-- Warehouse -->
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Kho nhập *</label>
            <select
          ...
          <!-- Rate (Optional) -->
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Đơn giá (tùy chọn)</label>
            <input
              ...
              step="100"
              class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-xh-red"
              placeholder="Để trống sẽ lấy giá mặc định"
            />
          </div>
...
onMounted(async () => {
  try {
    warehouses.value = await xuanhoaAPI.getWarehouses(0) // is_group = 0
  } catch (e) {
...
const handleSubmit = async () => {
  ...
  try {
    const result = await xuanhoaAPI.createMaterialReceipt(
      form.item_code,
      form.qty,
      form.warehouse,
      form.rate
